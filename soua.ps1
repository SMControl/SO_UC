...
# ---
# Version 1.46
# - Added check for running Smart Office processes in Part 10 before continuing.
...
Read-Host
# Check for Running Smart Office Processes
$processesToCheck = @("Sm32Main", "Sm32")
$SmartOfficeRunning = $false
foreach ($process in $processesToCheck) {
    if (Get-Process -Name $process -ErrorAction SilentlyContinue) {
        $SmartOfficeRunning = $true
    }
}
while ($SmartOfficeRunning) {
    Write-Host "Smart Office is running. Please close it and press Enter to continue." -ForegroundColor Red
    Read-Host
    $SmartOfficeRunning = $false
    foreach ($process in $processesToCheck) {
        if (Get-Process -Name $process -ErrorAction SilentlyContinue) {
...
# Part 12 - Revert Services and Processes to Original State
# -----
